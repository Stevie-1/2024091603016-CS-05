...
void getExpression(PFN *pfn1, PFN *pfn2, char *operator){
    char expression[263]; 
    printf("输入一个表达式："); 
    scanf("%262[^\n]", expression); 
    ...
    while (expression[location] != '.'){
        pfn1->integer[location-carry] = expression[location];
        location++;
    } 
    pfn1->integer[location+1-carry] = '\0'; // pfn1->integer的终止
    location++; 
    carry = location;

    //获取第一个定点数的小数部分
    while (expression[location]!= '+' && expression[location]!= '-' && expression[location]!= '*' && expression[location]!= '/'){
        pfn1->fraction[location-carry] = expression[location];
        location++;
    } 
    while (1){ //pfn1->fraction的终止 
        if (pfn1->fraction[location-carry-1] == ' '){
            carry++;
        }
        else{
            break;
    ...
    if (pfn1->fraction[location-carry-1] == ')'){ 
        pfn1->system = pfn1->fraction[location-2-carry];
        pfn1->fraction[location-carry-2] = '\0';
    } 
    else{ 
        pfn1->system = pfn1->fraction[location-carry-1];
        pfn1->fraction[location-carry-1] = '\0';
    }
    //获取运算符
...
int main(){ 
    ...
    //从表达式中获取操作数和操作符，并把操作数转化为十进制
    getExpression(&pfn1, &pfn2, &operation); 
    initPFNAndStoreInDecimal(&pfn1); 
    initPFNAndStoreInDecimal(&pfn2);

    //根据操作符进行运算，得到结果
    if (operation == '+'){
        PFN *result = PFNAdd(&pfn1, &pfn2);
        printPFN(*result);
    } 

    if (operation == '-'){
        PFN *result = PFNSub(&pfn1, &pfn2);
        printPFN(*result);
    }

    if (operation == '*'){
        PFN *result = PFNMul(&pfn1, &pfn2);
        printPFN(*result);
    }
    return 0;
}